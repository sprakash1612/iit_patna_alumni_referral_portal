...
import { Search, SendHorizontal, X, Loader2, Users, Mail, Phone, Building2 } from 'lucide-react'
  ...
  const sendReferral = async () => {
    ...
    try {
      ...
      const body = encodeURIComponent(
        `Hi ${modal.name},\n\nI am ${user.name}, an IIT Patna alumnus. I came across your profile on the IITP Referral Portal and would like to request a referral${modal.current_company ? ` at ${modal.current_company}` : ''}.\n\n` +
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1">
        <div className="bg-gradient-to-r from-brand-800 to-brand-600 rounded-2xl p-6 mb-8 text-white">
          <h2 className="text-xl font-bold">Alumni Directory</h2>
          <p className="text-blue-100 text-sm mt-1">Find IIT Patna alumni and request referrals.</p>
        </div>
                    ...
                    {filtered.map(u => (
                      <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-5 py-4">
                          ...
                          {u.personal_email && <p className="text-gray-400 text-xs flex items-center gap-1 mt-0.5"><Mail size={10} />{u.personal_email}</p>}
                          {u.mobile && <p className="text-gray-400 text-xs flex items-center gap-1 mt-0.5"><Phone size={10} />{u.show_mobile ? u.mobile : maskMobile(u.mobile)}</p>}
                        </td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex flex-wrap gap-1">
                            {(u.skills || []).length > 0 ? u.skills.slice(0, 3).map(s => <SkillBadge key={s} skill={s} />) : <span className="text-gray-400">—</span>}
                ...
                <div key={u.id} className="card">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {u.personal_email && <p className="text-gray-400 text-xs mt-0.5">{u.personal_email}</p>}
                      {u.mobile && <p className="text-gray-400 text-xs mt-0.5">{u.show_mobile ? u.mobile : maskMobile(u.mobile)}</p>}
                    </div>
                    {sentIds.has(u.id) ? (
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 z-10 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">Requesting from <span className="font-semibold text-gray-700">{modal.name}</span>{modal.current_company ? ` @ ${modal.current_company}` : ''}</p>
              </div>
