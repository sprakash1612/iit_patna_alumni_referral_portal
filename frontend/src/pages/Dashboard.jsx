...
export default function Dashboard() {
  ...
  const openModal = (targetUser) => { setModal(targetUser); setMessage('') }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-gradient-to-r from-brand-800 to-brand-600 rounded-2xl p-6 mb-8 text-white">
          ...
          <p className="text-blue-100 text-sm mt-1">Find IIT Patna alumni and request referrals at their companies.</p>
        </div>
                      ...
                      <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-5 py-4">
                          ...
                          {u.personal_email && (
                            <p className="text-gray-400 text-xs flex items-center gap-1 mt-0.5">
                              <Mail size={10} />{u.personal_email}
                            </p>
                          )}
                          {u.mobile && (
                            <p className="text-gray-400 text-xs flex items-center gap-1 mt-0.5">
                              <Phone size={10} />
                              {u.show_mobile ? u.mobile : u.mobile}
                            </p>
                          )}
                        </td>
                        <td className="px-5 py-4">
                          ...
                          {u.current_company && (
                            <p className="text-gray-500 text-xs flex items-center gap-1 mt-0.5">
                              <Building2 size={10} />{u.current_company}
                            </p>
                          )}
                          {(u.previous_company || []).length > 0 && (
                            <p className="text-gray-400 text-xs mt-0.5">
                              Prev: {u.previous_company.join(', ')}
                            </p>
                          )}
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex flex-wrap gap-1">
                            ...
                            {(u.skills || []).length > 3 && (
                              <span className="text-xs text-gray-400 self-center">+{u.skills.length - 3}</span>
                            )}
                          </div>
                        </td>
                    ...
                    <div className="min-w-0">
                      ...
                      {u.designation && (
                        <p className="text-gray-600 text-sm mt-1">
                          {u.designation}{u.current_company ? ` @ ${u.current_company}` : ''}
                        </p>
                      )}
                      {(u.previous_company || []).length > 0 && (
                        <p className="text-gray-400 text-xs mt-0.5">Prev: {u.previous_company.join(', ')}</p>
                      )}
                      ...
                      {u.mobile && <p className="text-gray-400 text-xs mt-0.5">{u.mobile}</p>}
                    </div>
                    {sentIds.has(u.id) ? (
